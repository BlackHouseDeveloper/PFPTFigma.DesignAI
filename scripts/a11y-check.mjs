...
function loadColorTokens() {
  ...
  const colorFile = path.join(TOKENS_DIR, 'colors.json');
  
  if (!fs.existsSync(colorFile)) {
    console.warn('⚠️  Warning: colors.json not found. Skipping accessibility checks.');
    console.log('Run "pnpm run figma:pull" first to fetch tokens from Figma.');
    return null;
  }

  const data = JSON.parse(fs.readFileSync(colorFile, 'utf-8'));
  console.log(`✅ Loaded ${Object.keys(data).length} color tokens`);
  return data;
}
